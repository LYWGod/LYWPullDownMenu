...
// 更新下拉菜单标题通知名称
NSString * const LYWUpdateMenuTitleNote = @"LYWUpdateMenuTitleNote";

@interface LYWPullDownMenu ()
/**
 *  下拉菜单所有按钮
 */
 ...
 *  下拉菜单蒙版
 */
@property (nonatomic, strong) LYWCover *coverView;
/**
 *  下拉菜单底部View
 */
...
@implementation LYWPullDownMenu
...
- (UIView *)coverView
{
    if (_coverView == nil) {
        ...
        _coverView = [[LYWCover alloc] initWithFrame:CGRectMake(coverX, coverY, coverW, coverH)];
...
- (void)setup
{
    ...
    // 监听更新菜单标题通知
    _observer = [[NSNotificationCenter defaultCenter] addObserverForName:LYWUpdateMenuTitleNote object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
